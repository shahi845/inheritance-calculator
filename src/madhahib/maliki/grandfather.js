 ...
 * Under the Mālikī school, he takes the best of:
 ...
 */
export function computeMalikiGrandfatherBestShare(heirs, context, remainderAfterOthers) {
    ...
    context.messages.push(
        `Mālikī Paternal Grandfather with siblings: ` +
 ...
 */
export function distributeMalikiSiblings(heirs, context, siblingsPool, shares) {
    const activeFull  = heirs.fullBrother    || 0;
    const activeFullS = heirs.fullSister     || 0;
    const activePat   = heirs.paternalBrother || 0;
    const activePatS  = heirs.paternalSister  || 0;
    ...
    if (activeFull > 0) {
        ...
        if (activePat > 0 || activePatS > 0) {
            context.messages.push(`Consanguine siblings blocked by Full Brother.`);
        }
        return;
    }
    ...
    if (activeFullS > 0) {
        // Full sister(s) can take up to their maximum fixed share of the whole estate
        const maxSisterFraction = activeFullS === 1 ? fraction(1, 2) : fraction(2, 3);

        // If the pool is smaller than or equal to their maximum share, they take the entire pool
        if (compareFractions(siblingsPool, maxSisterFraction) <= 0) {
            const sh = siblingsPool;
            shares.push({
                heir: 'fullSister',
                name: 'Full Sister',
                count: activeFullS,
                ...
                adjustedShare: sh,
                status: 'Residuary',
                reason: `Residuary — Grandfather-with-siblings pool (entire pool taken by Full Sister(s) up to Qur'anic limit)`
            });
            context.messages.push(`Full Sister(s) take the entire sibling pool of ${sh.num}/${sh.den}.`);
